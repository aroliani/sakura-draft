...
export default function DashboardCard({ title, value, icon: Icon, onClick, variant = "default" }: Props) {
  return (
    <button
      onClick={onClick}
      aria-label={title}
      className={`bg-card rounded-xl border-2 p-5 text-left transition-all hover:shadow-lg hover:-translate-y-0.5 ${VARIANT_STYLES[variant]}`}
    >
      <div className="flex items-center justify-between">
        <div className={`w-11 h-11 rounded-lg flex items-center justify-center ${ICON_STYLES[variant]}`}>
          <Icon size={22} />
        </div>
