      ...
      <div className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/40 backdrop-blur-sm animate-fade-in p-4" onClick={onClose}>
        <div className="bg-card rounded-xl shadow-2xl w-full max-w-3xl max-h-[90vh] flex flex-col animate-fade-in" onClick={(e) => e.stopPropagation()}>
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-6 space-y-6 scrollbar-thin">
            ...
            {(doc.status === "Disetujui" || doc.status === "Diarsipkan") && (<div className="p-4 rounded-xl border border-border bg-muted/30"><div className="flex items-center gap-2 mb-3"><QrCode size={18} className="text-primary" /><h3 className="font-bold text-foreground text-sm">QR Verifikasi</h3></div><div className="flex items-center gap-4"><div className="bg-background p-2 rounded-lg border border-border"><QRCodeSVG value={`${window.location.origin}/verify/${doc.id}`} size={96} level="M" includeMargin={false} /></div><div className="flex-1 text-sm space-y-1"><p className="text-muted-foreground text-xs">Scan QR code untuk memverifikasi keaslian dokumen ini.</p><a href={`/verify/${doc.id}`} target="_blank" rel="noopener noreferrer" className="text-xs text-primary font-medium hover:underline">Buka halaman verifikasi →</a></div></div></div>)}
          </div>
        </div>
      </div>
