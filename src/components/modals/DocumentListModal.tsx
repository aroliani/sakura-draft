...
export default function DocumentListModal({ title, documents, onClose, onSelectDocument }: Props) {
  return (
          ...
          {documents.map((doc) => (
            ...
            >
              ...
              <div className="flex flex-col items-end gap-1 shrink-0">
                <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${STATUS_COLORS[doc.status]}`}>{doc.status}</span>
                <span className="text-xs text-muted-foreground">{format(new Date(doc.tanggalUpload), "dd/MM/yyyy")}</span>
              </div>
            </button>
          ))}
        </div>
