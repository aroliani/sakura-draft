    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-foreground/40 backdrop-blur-sm animate-fade-in" onClick={onClose}>
      <div className="bg-card rounded-xl shadow-2xl w-full max-w-md animate-fade-in" onClick={(e) => e.stopPropagation()}>
        <div className="flex items-center justify-between px-6 py-4 border-b border-border">
          <h2 className="font-bold text-foreground">Profil Pengguna</h2>
        ...
        <div className="p-6 space-y-5">
          ...
          <div className="flex items-center gap-4">
            <div className="relative">
              <img src={user.avatar} alt="" className="w-16 h-16 rounded-full object-cover border-2 border-primary/20" />
            </div>
            <div>
              <div className="font-bold text-foreground">{user.nama}</div>
              <span className="text-xs px-2 py-0.5 rounded-full bg-primary text-primary-foreground font-medium">{user.role}</span>
            </div>
          </div>
          ...
          {!showCamera && (
            <div className="flex gap-2">
              ...
              <button onClick={startCamera} className="flex-1 flex items-center justify-center gap-2 px-3 py-2 rounded-lg border border-input text-sm hover:bg-muted transition-colors">
          ...
          {showCamera && (
            <div className="space-y-2">
              <video ref={videoRef} autoPlay playsInline className="w-full rounded-lg bg-foreground/5" />
              <canvas ref={canvasRef} className="hidden" />
              <div className="flex gap-2">
                <button onClick={capturePhoto} className="flex-1 px-3 py-2 rounded-lg bg-primary text-primary-foreground text-sm font-medium">Ambil Foto</button>
                <button onClick={() => { stream?.getTracks().forEach((t) => t.stop()); setShowCamera(false); setStream(null); }} className="px-3 py-2 rounded-lg border border-input text-sm">Batal</button>
              </div>
            </div>
          )}
          ...
          <div className="space-y-4">
            {[
              { icon: UserIcon, label: "Nama Lengkap", value: user.nama },
              { icon: Mail, label: "Email", value: user.email },
              { icon: Shield, label: "Role", value: user.role },
              { icon: Building, label: "Departemen", value: user.departemen },
